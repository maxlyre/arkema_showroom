<script setup>
  ...
  const emit = defineEmits(['pointerup','pointerdown'])
    ...
    void main() {
        ...
        perlinCol = clamp(perlinCol , 0.0 , 1.0);

        vec3 colStart = normalizeRGB(vec3(211.0, 0, 0));
        vec3 colEnd = normalizeRGB(vec3(200.0, 0, 0));
        vec3 col = mix(colStart, colEnd, perlinCol);
        gl_FragColor = vec4(col.rgb,perlinCol.x);
    }`;
...
<template>
  <div id="buttonKynar">
    ...
    <div id="button" @pointerdown="pressDown" @pointerleave="pressUp">
      <img class="icon" src="../assets/icon_red.svg" alt="">
